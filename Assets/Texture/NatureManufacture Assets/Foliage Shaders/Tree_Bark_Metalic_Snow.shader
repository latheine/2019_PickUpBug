	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float2 uv_DetailNormalMap = i.uv3_texcoord3 * _DetailNormalMap_ST.xy + _DetailNormalMap_ST.zw;
				...
				float2 staticSwitch149 = uv_DetailNormalMap;
			#else
				float2 staticSwitch149 = uv_DetailAlbedoMap;
			#endif
			float2 uv_DetailMask = i.uv_texcoord * _DetailMask_ST.xy + _DetailMask_ST.zw;
			float4 tex2DNode25 = tex2D( _DetailMask, uv_DetailMask );
			float3 lerpResult19 = lerp( UnpackScaleNormal( tex2D( _BumpMap, uv_MainTex ), _BumpScale ) , UnpackScaleNormal( tex2D( _DetailNormalMap, staticSwitch149 ), _DetailNormalMapScale ) , tex2DNode25.a);
			...
			float2 uv_SnowCoverNormal = i.uv3_texcoord3 * _SnowCoverNormal_ST.xy + _SnowCoverNormal_ST.zw;
				...
				float2 staticSwitch154 = uv_SnowCoverNormal;
			#else
				float2 staticSwitch154 = uv_SnowCover;
			#endif
			...
			float4 lerpResult16 = lerp( tex2D( _MainTex, uv_MainTex ) , tex2D( _DetailAlbedoMap, staticSwitch149 ) , tex2DNode25.a);
			...
			float4 lerpResult18 = lerp( tex2D( _MetalicRAOGSmothnessA, uv_MainTex ) , tex2D( _DetailMetalicRAOGSmothnessA, staticSwitch149 ) , tex2DNode25.a);
