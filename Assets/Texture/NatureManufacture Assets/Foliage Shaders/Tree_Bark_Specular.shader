Shader "NatureManufacture Shaders/Trees/Tree Bark Specular"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float2 uv_DetailNormal = i.uv3_texcoord3 * _DetailNormal_ST.xy + _DetailNormal_ST.zw;
			#ifdef _DETALUSEUV3_ON
				float2 staticSwitch123 = uv_DetailNormal;
			#else
				float2 staticSwitch123 = uv_DetailAlbedoMap;
			#endif
			...
			float4 lerpResult18 = lerp( tex2D( _SpecularRGBSmothnessA, uv_MainTex ) , tex2D( _DetailSpecularRGBSmothnessA, staticSwitch123 ) , tex2DNode25.a);
