Shader "NatureManufacture Shaders/Trees/Cross Model Shader Snow"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 tex2DNode3 = UnpackScaleNormal( tex2D( _BumpMap, uv_MainTex ), _BumpScale );
			o.Normal = tex2DNode3;
			...
			float3 lerpResult46 = lerp( tex2DNode3 , UnpackScaleNormal( tex2D( _SnowNormalRGB, uv_SnowAlbedoRGB ), _SnowNormalPower ) , saturate( ( ase_worldNormal * _Snow_Amount ) ));
			float4 tex2DNode68 = tex2D( _SnowMaskA, uv_MainTex );
			...
			float4 lerpResult56 = lerp( ( tex2DNode2 * _HealthyColor ) , ( tex2D( _SnowAlbedoRGB, uv_SnowAlbedoRGB ) - float4( appendResult76 , 0.0 ) ) , lerpResult150);
