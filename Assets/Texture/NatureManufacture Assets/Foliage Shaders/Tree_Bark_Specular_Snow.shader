	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float2 uv_DetailNormalMap = i.uv3_texcoord3 * _DetailNormalMap_ST.xy + _DetailNormalMap_ST.zw;
				...
				float2 staticSwitch157 = uv_DetailNormalMap;
			#else
				float2 staticSwitch157 = uv_DetailAlbedoMap;
			#endif
			float2 uv_DetailMask = i.uv_texcoord * _DetailMask_ST.xy + _DetailMask_ST.zw;
			float4 tex2DNode25 = tex2D( _DetailMask, uv_DetailMask );
			float3 lerpResult19 = lerp( UnpackScaleNormal( tex2D( _BumpMap, uv_MainTex ), _BumpScale ) , UnpackScaleNormal( tex2D( _DetailNormalMap, staticSwitch157 ), _DetailNormalMapScale ) , tex2DNode25.a);
			float2 uv_SnowCover = i.uv_texcoord * _SnowCover_ST.xy + _SnowCover_ST.zw;
			float2 uv_SnowCoverNormal = i.uv3_texcoord3 * _SnowCoverNormal_ST.xy + _SnowCoverNormal_ST.zw;
			#ifdef _SNOWUSEUV3_ON
				float2 staticSwitch160 = uv_SnowCoverNormal;
			#else
				float2 staticSwitch160 = uv_SnowCover;
			#endif
			...
			float4 lerpResult16 = lerp( tex2D( _MainTex, uv_MainTex ) , tex2D( _DetailAlbedoMap, staticSwitch157 ) , tex2DNode25.a);
			...
			float4 lerpResult18 = lerp( tex2D( _SpecularRGBSmothnessA, uv_MainTex ) , tex2D( _DetailSpecularRGBSmothnessA, staticSwitch157 ) , tex2DNode25.a);
			...
			float clampResult64 = clamp( tex2D( _AmbientOcclusionA, uv_MainTex ).g , ( 1.0 - _AmbientOcclusionPower ) , 1.0 );
