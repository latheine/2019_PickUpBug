...
public class FootstepMaster_Curves : MonoBehaviour
{
    ...
    void PlayDirt()
    {        //Supports toggling or instantiating a "dirtFX" AT THE PLAYER'S CURRENT FOOT which could be a dust cloud particle system, or something...
        currentVolume = (volume + UnityEngine.Random.Range(-volumeVariance, volumeVariance));
        pitch = (1.0f + Random.Range(-pitchVariance, pitchVariance));
        mySound.pitch = pitch;
        if (dirt.Length > 0)
    ...
    void PlaySnow()
    {        //Supports toggling or instantiating a "snowFX" AT THE PLAYER'S CURRENT FOOT which could be a white poofy particle system, or something...
        currentVolume = (volume + UnityEngine.Random.Range(-volumeVariance, volumeVariance));
        pitch = (1.0f + Random.Range(-pitchVariance, pitchVariance));
        mySound.pitch = pitch;
        if (snow.Length > 0)
    ...
    void PlayWater()
    {       //Supports toggling or instantiating a "waterFX" AT THE SURFACE OF THE WATER, which could be a splashy particle system, or something...
        currentVolume = (volume + UnityEngine.Random.Range(-volumeVariance, volumeVariance));
        pitch = (1.0f + Random.Range(-pitchVariance, pitchVariance));
        mySound.pitch = pitch;
        if (water.Length > 0)
